...
#define BUPT_EDU_SERVER "127.0.0.7"

int main()
{
    cJSON *rrJSONarray = readRRArray("../data/eduRR.json");
    int server_socket, client_socket;
    struct sockaddr_in server_addr, client_addr;
    server_socket = socket(AF_INET, SOCK_STREAM, 0);
    server_addr.sin_family = AF_INET;
    server_addr.sin_addr.s_addr = inet_addr(EDU_SERVER);
        ...
        *suf = '.';
        ...
        if (suffix != NULL)
        {
            if (strcmp(suffix, ".bupt.edu") == 0)
            {
                ...
                cJSON *comA = getResultArraybyName(rrJSONarray, "dns.bupt.com", A); // 从json中获取com服务器的A记录
                printf("array size: %d\n", cJSON_GetArraySize(comA));
                cJSON_Delete(comA);
                //DNS_RR *comArr = praseResult(comA);
                DNS_RR* comArr = malloc(sizeof(DNS_RR));
                name = htons(0xc000+compptr-2);
                uint32_t comip = inet_addr(BUPT_EDU_SERVER);
                comArr->type = htons(A);
